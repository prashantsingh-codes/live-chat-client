  ...
  useEffect(() => {
    ...
    socket.on("message received", (newMessage) => {
      if (newMessage.chat._id === chat_id) {
        setAllMessages((prev) => [...prev, newMessage]);
        setRefresh((prev) => !prev); // ← also refresh sidebar for current chat
      } else {
        setRefresh((prev) => !prev); // ← functional form, no stale closure
      }
    });
    return () => {
      socket.off("message received");
    };
  }, [chat_id]); // ← `refresh` removed from deps intentionally
